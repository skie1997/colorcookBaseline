...
import { getStackedData } from '../RadarChart/helper';
    ...
    if(('color' in encoding) && !_.isEmpty(encoding.color)){
        if(encoding.color.type === "quantitative"){
            ...
            points.attr("fill", function(d,i) {
                console.log('data-scatter',d)
                if(colorset.length == 2 ){
                 
                    return computeColor(valueScale(d[encoding.size.field]))
                }else if(colorset.length == 3){
                    return computeColor(d[encoding.size.field])
                }
                else{
                    return globalColorscale(seriesIndexScale(i))
                }
